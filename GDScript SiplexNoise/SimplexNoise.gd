...
var CPU_SimplexNoise := GDScript_SimplexNoise.new()
...
func __render(p):
	if !Engine.get_main_loop():
		return
	yield(_render(),"completed") if Use_GPU else render_with_CPU()
...
func render_with_CPU():
	...
	for x in size.x:
		for y in size.y:
			var value : float = CPU_SimplexNoise.get_noise_2d(Vector2(x,y))
			image.set_pixel(x,y,Color( value, value, value))
	image.unlock()
	"""
	self.create_from_image(image)

func thread_rend(data:Dictionary):#For some reason, we have to pass the argms of the thead as a unique variable    #from_y:float,to_y:float,x_size:float,CPU_SimplexN:GDScript_SimplexNoise):
	var from_y : float = data["from_y"]
	var to_y : float = data["to_y"]
	var x_size : float = data["x_size"]
	var CPU_SimplexN:GDScript_SimplexNoise = data["CPU_SimplexN"]
	var result : PoolRealArray
	for _y in  (to_y - from_y):
		var y : float = _y + from_y
		for x in x_size:
			result.append(CPU_SimplexN.get_noise_2d(Vector2(x,y)))
	return result
